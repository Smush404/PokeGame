...
static void dijkstra_path_hiker(map_t *m)
{
  ...
  for (y = 1; y < MAP_Y - 1; y++) {
    for (x = 1; x < MAP_X - 1; x++) {
      if (path[y][x].cost < SHRT_MAX && world.cur_map->map[y][x] != ter_water){ //cost of terrain is not infinity
        path[y][x].hn = heap_insert(&h, &path[y][x]);
      }
      else{ 
        path[y][x].hn  = NULL; 
  ...
  while ((p = heap_remove_min(&h))) {
    ...
    if ((path[p->pos[dim_y] - 1][p->pos[dim_x]    ].hn) &&
        (path[p->pos[dim_y] - 1][p->pos[dim_x]    ].cost > (p->cost) )){ //N
    ...
    (path[p->pos[dim_y]    ][p->pos[dim_x] - 1].cost > ((p->cost )))){ //W
    ...
    if ((path[p->pos[dim_y]    ][p->pos[dim_x] + 1].hn) && 
        (path[p->pos[dim_y]    ][p->pos[dim_x] + 1].cost > ((p->cost)))) {//E
    ...
    if ((path[p->pos[dim_y] + 1][p->pos[dim_x]    ].hn) && 
        (path[p->pos[dim_y] + 1][p->pos[dim_x]    ].cost > ((p->cost + heightpair(p->pos))))){ //S
