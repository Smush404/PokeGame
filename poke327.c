...
// //TODO broken
// int cost_hiker(terrain_type_t *c, int m){
//   int ccost;

//   if(c == ter_boulder || c == ter_mountain){ccost = 10;}
//   if(c == ter_tree || c == ter_forest){ccost = 50;}
//   if(c == ter_path || c == ter_gate){ccost = 5;}
//   if(c == ter_mart || c == ter_center){ccost = 10;}
//   if(c == ter_grass || c == ter_clearing){ccost = 15;}
//   if(c == ter_water){ccost = 5;}

//   return ccost + m;
// }

static void dijkstra_path_hiker(map_t *m, pair_t from)
{
  static path_t path[MAP_Y][MAP_X], *p;
  static uint32_t initialized = 0;
  int cost[MAP_Y][MAP_X];
  heap_t h;
  uint32_t x, y;
  ...
  while ((p = heap_remove_min(&h))) {
    ...
    if ((path[p->pos[dim_y] - 1][p->pos[dim_x]    ].hn)){ //N
      ...
      path[p->pos[dim_y] - 1][p->pos[dim_y] - 1].from[dim_y] = p->pos[dim_y];
      path[p->pos[dim_y] - 1][p->pos[dim_x]    ].from[dim_x] = p->pos[dim_x];

      heap_decrease_key_no_replace(&h, path[p->pos[dim_y] - 1][p->pos[dim_x]    ].hn);

      cost[p->pos[dim_y]][p->pos[dim_y]] = 5;
    }
    ...
    if ((path[p->pos[dim_y]    ][p->pos[dim_x] - 1].hn)){ //w
      ...
      cost[p->pos[dim_y] - 1][p->pos[dim_y]    ] = 5;
    }
...
int main(int argc, char *argv[])
{
  ...
  do {
    ...
    //path prints
    dijkstra_path_hiker(world.cur_map, from);
    // print_hiker(world.cur_map->map);
